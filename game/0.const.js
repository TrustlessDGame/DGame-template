...
const checkExistWalletOutside = () => {
  const prvKeyLocal = localStorage.getItem(ACCOUNT_KEY);
  const addressLocal = localStorage.getItem(ADDRESS_KEY);
  const passLocal = localStorage.getItem(PASS_WORD);

  if (prvKeyLocal && addressLocal && passLocal) {
    return {
      [ACCOUNT_KEY]: prvKeyLocal,
      [ADDRESS_KEY]: addressLocal,
      [PASS_WORD]: passLocal,
    };
  }
...
const reGeneratePrivateKey = (cipherText, password) => {
  const generatePrvKey = decryptAES(cipherText, password);

  console.log({ generatePrvKey });

  const finalPassword = password + SALT_PASS;
  const hashPrvKey = encryptAES(generatePrvKey, finalPassword);
...
class WalletData {
  ...
  _loadModalActions() {
    ...
    modalActions.setAttribute("id", "modal-actions");
  ...
  async _checkLogin() {
    ...
    try {
      ...
      if (walletData) {
        ...
        const existWallet = checkExistWalletOutside();
        if (existWallet) {
          // Clone exist data wallet
          let walletData = {};
          walletData[ACCOUNT_KEY] = reGeneratePrivateKey(
            existWallet[ACCOUNT_KEY],
            existWallet[PASS_WORD]
          );
          walletData[ADDRESS_KEY] = existWallet[ADDRESS_KEY];
          walletData[PASS_WORD] = encryptAES(existWallet[PASS_WORD], SALT_PASS);
...
class ContractInteraction {
  ...
  loadContract(abiJson, contractAddress) {
    if (
      ...
      (!this.WalletData?.Balance && !PRACTICE_MODE)
    )
      return;
  ...
  async Send(
    ...
    if (
      !PRACTICE_MODE &&
      (!wallet?.Balance || Number(transactionCost) > Number(wallet?.Balance))
    ) {
